    ...
    const fetchWeatherData = async () => {
      if (filteredCities.length > 0) {
        try {
          ...
          while (
            newWeatherData.length < citiesPerPage &&
            index < filteredCities.length
          ) {
  ...
  const displayedCities = filteredCities
    .map((city) => {
      const weather = weatherData.find(
        (w) => w.name.toLowerCase() === city.toLowerCase()
      );
          ...
          <table>
            ...
            <tbody>
              {displayedCities.map((city, index) => (
                <tr key={index}>
                  <td>
                    <button onClick={() => setSelectedCity(city.name)}>
                      {city.name}
                    </button>
                  </td>
